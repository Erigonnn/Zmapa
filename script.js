...
function initGame() {
    ...
    navigator.geolocation.watchPosition(pos => {
        ...
        if (firstFix) { map.setView(p, 18); firstFix = false; }
        cleanupFarObjects(p);
...
window.doCraft = async (type, cost) => {
    ...
    if (type === 'base') {
        if (state.wood >= cost && !state.hasBase) {
            ...
            // Zapis do Firebase (Globalne bazy widoczne dla wszystkich)
            try {
...
document.getElementById('btn-attack').onclick = (e) => {
    ...
    // Nóż zwiększa zasięg ataku z 70 do 120 metrów
    const range = (state.weapon === "NÓŻ") ? 120 : 70;
    
    zombies = zombies.filter(z => {
        if(map.distance(pPos, z.getLatLng()) < range) { 
            map.removeLayer(z); 
            killed++; 
            return false; 
        }
