...
function initGame() {
    ...
    // GPS I AUTO-GENERATOR
    navigator.geolocation.watchPosition(pos => {
        const p = [pos.coords.latitude, pos.coords.longitude];
        playerMarker.setLatLng(p);
        map.panTo(p);

        // Generuj falę co 35 metrów marszu
    ...
    for(let i=0; i<4; i++) {
        ...
        loot.on('click', () => {
            if(map.distance(playerMarker.getLatLng(), loot.getLatLng()) < 45) {
                ...
                state.scrap += 5; state.wood += 3;
                showMsg("+5⚙️ +3🪵"); updateUI(true);
            } else showMsg("ZA DALEKO!");
        });

        if(i < 2) { // 2 zombie na falę
            const z = L.marker([p[0]+off(), p[1]+off()], { 
...
function gameLoop() {
    ...
    zMarkers.forEach(z => {
        ...
        if(dist < 100) { // ŚLEDZENIE
            ...
            z.setLatLng([zPos.lat + (Math.random()-0.5)*0.00002, zPos.lng + (Math.random()-0.5)*0.00002]);
        }
    });
}
...
document.getElementById('btn-attack').onclick = () => {
    ...
    zMarkers = zMarkers.filter(z => {
        if(map.distance(p, z.getLatLng()) < 45) {
            map.removeLayer(z); state.scrap += 4;
            showMsg("ZOMBIE ZLIKWIDOWANE (+4⚙️)"); updateUI(true);
            return false;
        }
        return true;
    });
